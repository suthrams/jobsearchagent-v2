...
- API-request observability is WIRED (ADR-074 Gap 5, net-new). The `@app.middleware("http")` in `app/api/main.py` records every REST request into the new `api_requests` table via `record_api_request_safe` (never-crash, runs in `finally`). It stores the matched ROUTE TEMPLATE (`/tailorings/{tailoring_id}`) — NEVER the raw path/query (PII-safe + bounded cardinality; unmatched routes -> `"<unmatched>"`), plus method/status/latency and the acting `user_id` (`?user_id=`, ADR-062). Read via `ApiRequestRepository.list_for_user` + `system_health.api_summary`; shown in the System Dashboard API section. Retention `observability_days` (independent window — no run FK). Covers the control path; UI browse reads bypass the API by design.
