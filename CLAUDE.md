...
- **ATS-direct sources (ADR-081; curated batch ADR-097):** `app/services/ats_scrapers.py` adds `GreenhouseScraper` + `LeverScraper` (source-of-truth employer feeds, boards fetched concurrently). Queried per company via `scrapers.{greenhouse,lever}.companies`; ships with a curated, live-verified default batch (ADR-097, 33 Greenhouse + 3 Lever) — re-verify/prune with `python tools/verify_ats_boards.py`. Built per run by `WorkflowDependencies.ats_scraper_factory(roles)`; additive alongside Adzuna. See `spike_job_data_sources.md`
