...
- ADR-074 status: Gaps 1-5 all implemented. Only two documented MINORS remain: ad-hoc resume-upload parse cost is unattributed (`parse_pdf(workflow_id=None)`), and `observability.md` historically listed methods that don't exist (largely corrected). Add a forcing-function test per newly-wired table.
