...
- **Fix the product, not the profile (generalize every fix)** — a defect or request surfaced by ONE job seeker's usage (e.g. profile 1's cleared-cyber search) is a *sample*, not the scope. Per-profile **configuration** is legitimately specific — that is the design (`effective_config`, ADR-062). Per-profile **logic, heuristics, or fixes are not** — never hardcode for the reporting profile, their role family, their industry, or the one posting they reported. Before implementing any fix: (1) **extract the general class** the observation reveals (BUG-010: "the field we parse can be truncated, so also read the field that cannot" — not "handle SOC watch-floor jobs"); (2) **review it against the whole application** — does this pattern recur for other profiles, sources, filters, or agents, and does the fix belong at a seam they all share; (3) **state explicitly** in the bug/ADR what the generalization covers AND what it deliberately does not, so the next reader sees the boundary. The reported case becomes the forcing-function **test**, never the target of the code. As job-seeker usage patterns accumulate, treat each as evidence about the product we are building for *any* job seeker — feed it back into the larger picture, don't bolt on a one-off. Rationale: BUG-010 arrived as a single cleared mid-level job for profile 1; the durable fix was a title-based recovery for the entire truncated-snippet class across all filters and sources.
- **ADR-first for architectural changes** — write/update the ADR in `docs/architecture/adr/` and any impacted docs **before** implementing. Skip for bug fixes, dep bumps, and refactors that don't change a contract. **Write ADRs concise and bullet-driven, not prose** — keep the section structure (Status, Context, Constraints, Decision, Decision review/not-a-rubber-stamp, How it integrates, Out of scope, PSSR, Tests, References) but express each section as terse bullets.
...
- **Render-verify every document change before committing (docs must not ship broken)** — a Markdown change is not "done" until you have confirmed it renders correctly on its target surface (GitHub for `README.md`/`docs/`/`wiki.md`/ADRs; Obsidian + LinkedIn for the blog articles). This is a required review step on ANY `.md` change, even a one-line edit. Checklist: (1) **Mermaid line breaks use `<br>`, never `\n`** — GitHub's mermaid renderer drops `\n` and can fail the whole diagram; same for `()`/`{}`/`·` inside labels, prefer quoted labels. (2) **Every referenced image path is git-tracked** — `git ls-files --error-unmatch <path>`; an untracked path is a broken image on GitHub (the gitignored `blogs/` PNGs are the deliberate exception, uploaded manually). (3) **Relative links resolve** to a committed file. (4) **Lists, code fences, and tables are well-formed** — blank line before a list/fence, no stray indentation, a `mermaid` fence inside `<details>` still needs its surrounding blank lines. For the LinkedIn articles, also run the Section-13 mechanical gate + the `grep -nE '!\[[^]]*\['` no-bracket-in-alt-text check from `blogs/blog_v2_series_guidelines.md`. When you cannot preview the rendered surface, say so and list exactly what you verified instead. Rationale: the README architecture diagram shipped with `\n` mermaid breaks and did not render on GitHub. See [[feedback_document_render_review]].
- **Run the test suite before committing** — `python -m pytest tests/` must pass; live-API tests are gated by `-m integration`. Strategy, layout, and the current count live in one place: `docs/testing.md` (don't hardcode counts elsewhere).
...
- **Durable run recovery across restarts (ADR-096):** runs execute in the in-process `_executor`; only `register_run` + `generate_report` write `workflow_runs`, so a process death mid-run freezes the row at `running`/`cancelling`. Layer 1 = graceful drain on lifespan shutdown (`drain_inflight_runs`, bounded by `WORKFLOW_SHUTDOWN_DRAIN_SECONDS`, default 30s); Layer 2 = checkpointed auto-resume on startup (`recover_orphaned_runs` re-submits via `_retry_graph` -> `graph.invoke(None, config)` under `MAX_RESUME_ATTEMPTS=3`, counter in `state_json.resume_attempts`); Layer 3 = `WorkflowRepository.reconcile_orphaned_runs()` fail-everything backstop. All runs submit through `_submit_run` so the drain can track them. Single-process assumption (multi-worker needs a shared registry). The drain intentionally does NOT shut down `_executor` (would break test-harness lifespan reuse; process exit tears it down anyway)
- **Fail-loud deployment guard (ADR-106):** the single-process + cooperative-trust + in-process-registry assumptions above are only safe as ONE worker bound to loopback. `app/api/deployment_guard.py::enforce_deployment_safety()` runs FIRST in the lifespan (before `build_and_cache_graph`, inside the `dependency_overrides` test-skip) and **refuses to start** (`UnsafeDeploymentError`) when it detects `WEB_CONCURRENCY>1` / `--workers >1` or a non-loopback `--host` from `sys.argv`+env. `ALLOW_UNSAFE_DEPLOYMENT` (truthy) downgrades to a logged warning. Best-effort tripwire (won't catch programmatic `uvicorn.run`/gunicorn-config launches) — it makes the cliff loud, it does NOT make the app multi-worker/exposure-safe. Detector is pure (`detect_unsafe_deployment(argv, environ)`) + has forcing-function tests
...
- **Adzuna per-minute rate limiter (ADR-107):** `ConcurrentAdzunaScraper` fans one call per task across 5 threads with no native governor, so a burst trips Adzuna's per-minute hits cap (the "20/25 hits per minute" alert). `app/services/rate_limiter.py` paces call STARTS to `scrapers.adzuna.max_calls_per_minute` (default 20, under the 25/min cap; 0 = off) via a PROCESS-GLOBAL singleton (`get_adzuna_limiter`) shared across all Adzuna instances incl. concurrent runs, and backs off on a `429`. Wired in `ConcurrentAdzunaScraper.make/scrape` (acquire per task in the worker, penalize on 429) — NO v1 modification (subclass-only)
- **Adzuna per-run call cap + partial results (ADR-108):** an unbounded role/location grid (e.g. 19 roles x 10 locations = ~209 calls) blows the per-minute AND daily quotas and exceeds the 180s discovery timeout (paced calls couldn't finish -> the all-or-nothing scrape lost EVERYTHING). `scrapers.adzuna.max_calls_per_run` (default 50, 0 = uncap) bounds calls/run; the kept subset is a Latin-square `_interleave_tasks` (broad title+location coverage); `scrape()` collects under a `_SCRAPE_TIME_BUDGET_S` (150s, below the 180s discovery timeout) via `as_completed(timeout=...)` and on `TimeoutError` returns PARTIAL results + `shutdown(wait=False, cancel_futures=True)` (stops spending calls). Reads `max_calls_per_run` from the run's effective config in both `dependencies.py` Adzuna paths. The downstream 50-discovered/10-scored funnel makes 50 calls plenty. **Per-run rotation (ADR-108 addendum):** the capped window is rotated by `(rotation_seed * cap) % total` so dropped combos are picked up on later runs; `rotation_seed = max(0, WorkflowRepository.count_for_user(user_id) - 1)` is computed in the `_adzuna_factory` (closes over `workflow_repo`; `discover_jobs` passes `user_id`) so it survives restarts. Best-effort (counts all run types) — eventually-complete, not strict round-robin
- **ATS-direct sources (ADR-081; curated batch ADR-097; per-profile ADR-098):** `app/services/ats_scrapers.py` adds `GreenhouseScraper` + `LeverScraper` (source-of-truth employer feeds, boards fetched concurrently). Queried per company via `scrapers.{greenhouse,lever}.companies`; ships with a curated, live-verified default batch (ADR-097, 33 Greenhouse + 3 Lever) — re-verify/prune with `python tools/verify_ats_boards.py`. **The company list is per-profile (ADR-098):** resolved PER RUN from `state["effective_config"]["scrapers"]` (not deps-time/global), so `discover_jobs` passes the run's scrapers config to `ats_scraper_factory(roles, scrapers_cfg)` and `build_ats_scrapers(roles, scrapers_cfg)` builds it; an edit applies next run with NO `/config/reload`. Managed entirely from the Settings "Target companies" section with **verify-on-add** (`POST /config/ats/verify` -> `ats_scrapers.verify_ats_board`); a profile override REPLACES the default list, a new profile inherits it. Additive alongside Adzuna. See `spike_job_data_sources.md`
- **Workday ATS-direct (ADR-101):** extends the same seam to **Workday** (the source-of-truth for cleared-gov + most F500; the root-cause fix for BUG-010's truncated-snippet class — full 5-8k-char JDs vs Adzuna's ~500). Lives in its OWN module `app/services/workday_scraper.py` (`WorkdayScraper`, `parse_workday_url`, `verify_workday_board`) because the two-phase list+detail fetch and the 3-part-id URL parsing/SSRF host-guard are too much to fold into `ats_scrapers.py`; the shared seam (`build_ats_scrapers`, `verify_ats_board`) **lazy-imports** it (one-way dep; reuses `_strip_html`/`_title_ok`). A board is a `{tenant, dc, site}` triple parsed from the pasted career URL — `parse_workday_url` is the single parsing source AND the only SSRF guard (`*.myworkdayjobs.com` only, enforced in BOTH the scraper and the verify endpoint). `scrapers.workday.companies` stores the triples (not flat slugs). Volume control = list (bounded pages) -> title-filter BEFORE any detail fetch -> capped detail fetch; never-lose-the-run on any board/detail failure. Ships **off** (empty default list) — per-profile opt-in. Settings add form pastes the URL; the verify response carries the parsed triple so the UI stores it without re-parsing
...
- **Dead-link filter (ADR-095, opt-in):** `search.drop_dead_links` drops postings whose apply link is VERIFIABLY dead (HTTP 404/410 or a closed-job body marker) via `app/services/dead_link_filter.py` (`check_link`/`filter_dead_links`), run in `discover_with_stats` after dedup. Network I/O (one bounded httpx GET/job, ≤10 concurrent, 4s timeout). Never-lose-the-run: keeps the job on timeout/429/DNS/connection/5xx (verdict "unknown"). Off by default; `discovery_stats.dead_link_dropped`
...
Phases 1-8 + post-8 work all complete. **Latest activity -> see `CHANGELOG.md`.** Current test counts + the test strategy/layout live in `docs/testing.md` (and CI); ADR counts in the ADR index. Don't hardcode any of these here — they go stale.
