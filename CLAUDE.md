...
- ADR-074 gaps STILL OPEN: out-of-graph runs get no `run_metrics` (Gap 3); deprecated thread-local `last_call_usage` in `custom_url_scraper` (Gap 4); and API-request observability (Gap 5 — net-new `api_requests` table + FastAPI middleware; the HTTP layer is currently unobserved). Wire through `ObservabilityService` when addressed; add a forcing-function test per newly-wired table.
