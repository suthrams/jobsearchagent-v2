...
- **Experience targeting (ADR-065, per-profile, opt-in).** `search.max_years_experience` (0/None = off) drops postings whose description states a minimum above the cap via `app/services/experience_filter.py::min_required_years` (deterministic regex, no LLM); postings with no detectable experience are kept. `search.exclude_senior` (bool) drops senior roles via Adzuna `what_exclude` (`SENIOR_TERMS`) and the per-run title gate. Both read from `effective_config.search` in `discover_jobs`. Read the cap via the node, never inline. Off by default so Primary is unaffected.
