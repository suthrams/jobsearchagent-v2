...
- ADR-074 catalogs the remaining gaps (still OPEN): `step_executions` dead (`log_step_*` unwired), out-of-graph runs get no `run_metrics`, and the deprecated thread-local `last_call_usage` in `custom_url_scraper`. Wire through `ObservabilityService` when addressed; add a forcing-function test per newly-wired table.
