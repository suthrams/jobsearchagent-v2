...
| Relevance axis | **Suitability, not keyword-match (prompt v4, field-agnostic)** — `unrelated` drops roles in a clearly different profession with no transferable overlap, NOT roles merely outside `target_roles`. Adjacency is DERIVED from the candidate's own `resume_profile` (no hardcoded industry); weighted by career stage so adjacent transferable-skill roles are KEPT. The cyber keep-list is a labelled example only — one shared prompt scales to any field without per-profile edits. |
| Truncated-text seniority (ADR-104, v4) | Aggregator snippets (Adzuna 500 chars) cut off the years/seniority requirement. For the **seniority axis only**, the prompt MAY use general world-knowledge of a role title/family's TYPICAL level when the text is silent, and marks `too_senior` if that typical level is materially above the candidate's `max_years` window. Field-agnostic ("do not assume any industry"). |
| Decision bias | **Axis-split (ADR-104).** Role-suitability: conservative, keep-when-unsure (recall-biased, mirrors ADR-065). Seniority for an **early-career** profile (`exclude_senior`/low `max_years`): precision-biased — drop `too_senior` on ambiguity, since over-experienced roles waste scoring budget. Non-early-career: seniority stays keep-when-unsure. |
